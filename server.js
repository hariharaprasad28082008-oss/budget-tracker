...
app.post("/api/login", (req, res) => {
    ...
    db.query(sql, [email], async (err, results) => {
        ...
        const user = results[0]; // FIXED: Correct row extraction
...
app.get("/api/summary", requireLogin, (req, res) => {
    ...
    db.query(sql, [req.session.userId], (err, results) => {
        ...
        const totalIncome = Number(results[0].totalIncome || 0); // FIXED: Correct row extraction
        const totalExpense = Number(results[0].totalExpense || 0); // FIXED: Correct row extraction
...
app.get("/", (req, res) => {
    ...
    res.sendFile(path.join(__dirname, "public", "login.html")); // FIXED: Forces Login Page First
});
