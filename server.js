...
function requireLogin(req, res, next) {
    if (!req.session || !req.session.userId) { return res.status(401).json({ error: "Please login first" }); }
    next();
}
...
app.post("/api/login", (req, res) => {
    ...
    db.query(sql, [email], async (err, results) => {
        ...
        const user = results; // FIXED: Handles array pool results correctly
        ...
        req.session.save((err) => {
            if (err) { console.error("Session save error:", err); return res.status(500).json({ error: "Session failed" }); }
